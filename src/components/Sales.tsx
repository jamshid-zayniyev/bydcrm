...
export function Sales() {
  ...
  const pipeline = [
    { stage: 'Новые лиды', count: customers.filter(c => c.status === 'new').length, color: 'blue' },
    { stage: 'Связались', count: customers.filter(c => c.status === 'contacted').length, color: 'purple' },
    { stage: 'Квалифицированы', count: customers.filter(c => c.status === 'qualified').length, color: 'green' },
    { stage: 'Переговоры', count: customers.filter(c => c.status === 'negotiation').length, color: 'yellow' },
    { stage: 'Продажи', count: customers.filter(c => c.status === 'won').length, color: 'red' }
  ];

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-gray-900 mb-1">Отдел продаж</h2>
        <p className="text-gray-500 text-sm">Управление сделками и воронка продаж</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div className="p-5 border-b border-gray-200">
          <h3 className="text-gray-900">Последние сделки</h3>
        </div>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-3 ml-13">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-700">{formatAmount(sale.amount)}</span>
                    </div>
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-700">{sale.salesPerson}</span>
                    </div>
                  </div>
                </div>

                <span className={`px-3 py-1 rounded-lg text-sm border ${statusColors[sale.status]}`}>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-white/90 mb-3">
              На основе анализа клиентов, рекомендуется сосредоточиться на лидах со статусом "Квалифицирован". 
              Вероятность конверсии в продажу составляет 65% в течение следующих 7 дней.
            </p>
